# UI State 规范（增量 / Delta Spec）

> **Capability**：`ui-state`
> **Change**：`add-zustand-stores`
> **类型**：ADDED Requirements（全新能力，不修改既有 spec）
> **来源**：`docs/architecture.md` §1.4 状态选型 + §3.2 数据流向 + §5.4 状态分层

本 capability 描述 Planote 状态层的**契约**——业务 store 与 UI store 如何切分、实体数据如何被订阅、错误如何被归一化、UI 偏好如何被持久化。**不描述** Repository 层如何实现（`plan-data` capability 范围），也不描述具体 UI 组件如何消费（下个 `add-data-binding-dashboard` change 范围）。
...
- 字段 / 枚举详细定义：`docs/architecture.md` §3.1
- Repository 接口签名：`docs/architecture.md` §4.1
- 状态分层：`docs/architecture.md` §5.4
- 错误类型：`docs/architecture.md` §4.3
- 关键模块数据流：`docs/architecture.md` §2.3（勾选事项→UI 多组件同步）
- useLiveQuery 详细：`dexie-react-hooks` 文档
- Zustand persist 中间件：`zustand/middleware` 文档
