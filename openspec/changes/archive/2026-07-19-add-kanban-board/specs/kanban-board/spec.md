# kanban-board 规范（增量 / Delta Spec）

> **Capability**：`kanban-board`
> **Change**：`add-kanban-board`
> **类型**：ADDED Requirements（全新能力）
> **来源**：PRD v1.0 §5.2「计划与博客一体化」（按状态浏览+操作 item）+ `docs/architecture.md` §3.1 Item 模型 + `add-plan-list-view`（视图模式参考）+ `add-plan-detail-view`（item 勾选 UI 参考）+ `add-app-shell`（已落地 EmptyState / Skeleton）

本 capability 定义 Planote 计划项看板 `/kanban` 的完整交互契约——4 列固定、跨列拖拽改状态、跨计划拖拽（保留 planId）、点击卡跳详情锚点。
...
- `Item` 模型：`src/types/domain.ts`（id / planId / title / status / dueDate / checked / order）
- `Item` 紧急度：通过 `Item.planId → Plan.urgency` 间接获取（v1.0 item 自身无 urgency 字段）
- `ItemRepo.list` / `update`：`src/db/repos/ItemRepo.ts`
- `useItemsStore.updateItem`：`src/stores/itemsStore.ts`（已落地，wrap ItemRepo.update）
- `useLiveQuery`：`dexie-react-hooks`（已落地）
- `usePlans`：`src/stores/hooks/usePlans.ts`
- `ItemRow.data-item-id`：本 change 要求 + `data-item-id={item.id}` 属性
- `EmptyState` / `Skeleton`：`src/components/shell/`
- `cn` / `formatChineseDate`：`src/lib/utils.ts`
- `useLocation`：`react-router-dom`（v6.4+）
- 视觉规范参考：`add-plan-list-view`（PlanCard）
- 跨 store 同步参考：`docs/architecture.md` §2.3
