# Dashboard Data 规范（增量 / Delta Spec）

> **Capability**：`dashboard-data`
> **Change**：`add-data-binding-dashboard`
> **类型**：ADDED Requirements（全新能力）
> **来源**：`docs/architecture.md` §1.4 + §5.4 + §6.1 + project.md §7.3

本 capability 描述 Dashboard 页面的**数据契约**——4 个数字卡、今日聚焦、即将到期、最近博客、最近活动如何从真实数据派生，**不描述** 视觉布局 / 颜色 / 动画（这些是 prototype/app.js 已锁定的视觉规范）。
...
- 字段 / 枚举定义：`docs/architecture.md` §3.1
- 紧急度算法：`docs/architecture.md` §6.4
- 派生 selector 模式：`docs/architecture.md` §5.4
- useLiveQuery hook 设计：`openspec/specs/ui-state/spec.md` Requirement: useLiveQuery hook 自动响应 IndexedDB 变化
- 业务 store 接口：`openspec/specs/ui-state/spec.md` 6 个 store 相关 Requirements
- Repository 接口签名：`docs/architecture.md` §4.1 + `openspec/specs/plan-data/spec.md`
