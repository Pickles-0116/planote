# blog-editor 规范（增量 / Delta Spec）

> **Capability**：`blog-editor`
> **Change**：`add-blog-tiptap-editor`
> **类型**：ADDED Requirements（全新能力）
> **来源**：PRD §6.5 博客编辑器 + architecture.md §4 编辑器

本 capability 定义 Tiptap 富文本编辑器在博客编辑/详情页的契约——内容格式、工具栏、自动保存、框架应用、只读模式、字数统计、旧数据迁移。所有"博客内容编辑"相关后续 change（附件、协作、导出）都基于本 capability 扩展。
...
- 现有 BlogEdit：`src/pages/blogs/BlogEdit.tsx`（被本 change 改造）
- 现有 BlogDetail：`src/pages/blogs/BlogDetail.tsx`（被本 change 改造）
- Blog 模型：`src/types/domain.ts` `Blog.content` 字段注释
- useBlogStore：`src/stores/useBlogStore.ts`（`updateBlog` action 复用）
- Framework 模型：`src/types/domain.ts` `Framework` / `FrameworkSection`
- useFrameworkStore：`src/stores/useFrameworkStore.ts`（提供框架 sections）
- 排序引擎：`src/shared/sort`（无关本 change；提及保持文档完整）
