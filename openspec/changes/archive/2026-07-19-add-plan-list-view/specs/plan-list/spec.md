# plan-list 规范（增量 / Delta Spec）

> **Capability**：`plan-list`
> **Change**：`add-plan-list-view`
> **类型**：ADDED Requirements（全新能力）
> **来源**：`docs/prd.md` §4.3（计划列表）+ `docs/ux-guidelines.md` §1 原则 4（密度自适配）+ `openspec/specs/plan-data/spec.md`（Plan 数据模型）

本 capability 描述 Planote 计划列表页 `/plans` 的完整交互契约——3 种视图模式（分组/全部/表格）、智能排序、全局搜索、折叠展开。所有后续 change（`add-plan-detail-view` / `add-plan-edit-form` / `add-plan-batch-ops`）在本规范定义的路由与数据 pipeline 上扩展。
...
- 紧急度派生计算（urgency 公式）：`openspec/specs/plan-data/spec.md` Requirement: 紧急度派生计算
- 进度派生计算（progress 公式）：`openspec/specs/plan-data/spec.md` Requirement: 进度派生计算
- 通用 UI Shell（EmptyState / Skeleton / LoadingOverlay）：`openspec/specs/ui-shell/spec.md`
- useLiveQuery 订阅约定：`openspec/specs/ui-state/spec.md`
- UI 状态 store（useUIStore）：`openspec/specs/ui-state/spec.md` Requirement: UI 状态持久化到 localStorage
- UX 原则 4 密度自适配：`docs/ux-guidelines.md` §1
- 计划列表原型：`docs/prototype/plans.html`
