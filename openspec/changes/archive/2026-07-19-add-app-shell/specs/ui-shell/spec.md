# UI Shell 规范（增量 / Delta Spec）

> **Capability**：`ui-shell`
> **Change**：`add-app-shell`
> **类型**：ADDED Requirements（全新能力）
> **来源**：`docs/architecture.md` §5.4（错误处理 / 加载约定）+ project.md §6（Sprint 1 验收）

本 capability 描述 Planote 应用的**通用外壳组件**——4 个展示级组件（ErrorBoundary / LoadingOverlay / EmptyState / Skeleton）以及路由级 Suspense + React.lazy 的**集成约定**。所有后续 change 在实现页面时可复用本规范定义的原语。
...
- 通用展示组件约定：`docs/architecture.md` §5.4「派生数据走 selector」
- 状态外壳模式（loading / empty / error）：`docs/architecture.md` §5.4 末段
- 现有 Card 组件：`src/components/ui/Card.tsx`（4 组件与其同级，目录 `shell/` 区分）
- useLiveQuery 异步态：`openspec/specs/ui-state/spec.md` Requirement: useLiveQuery hook 自动响应 IndexedDB 变化
- 路由结构：`src/App.tsx` + `docs/architecture.md` §5.3
- 项目目录约定（`@/components`）：`openspec/project.md` §4
