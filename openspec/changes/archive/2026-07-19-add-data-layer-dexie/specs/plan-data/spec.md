# Plan Data 规范（增量 / Delta Spec）

> **Capability**：`plan-data`
> **Change**：`add-data-layer-dexie`
> **类型**：ADDED Requirements（全新能力，不修改既有 spec）
> **来源**：`docs/architecture.md` §3 数据模型 + §4 API 设计

本 capability 描述 Planote 数据层的**契约**——所有数据如何被持久化、如何被查询、如何被派生计算。**不描述** UI 层如何展示，也不描述 Store 如何订阅（这些是后续 change 的范围）。
...
- 索引设计：`docs/architecture.md` §3.2
- Repository 接口签名：`docs/architecture.md` §4.1
- 错误类型：`docs/architecture.md` §4.3
- 紧急度算法描述：`docs/architecture.md` §6.4
- 4 套框架章节内容（待定）：从 `prototype/pages/blog-edit.html` 的 framework 区块反查（design.md 给出逐套字段）
