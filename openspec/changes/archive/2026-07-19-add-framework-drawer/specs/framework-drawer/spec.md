# framework-drawer 规范（增量 / Delta Spec）

> **Capability**：`framework-drawer`
> **Change**：`add-framework-drawer`
> **类型**：ADDED Requirements（全新能力）
> **来源**：PRD v1.0 框架库降级为抽屉 + architecture.md §6.5 框架抽屉方案

本 capability 定义"框架库抽屉"在博客编辑器侧的契约——抽屉壳、预置数据、搜索、tag 筛选、选中、应用、与 `useApplyFramework` 集成、a11y 简化。
...
- 现有 `useApplyFramework`：`src/features/blog/hooks/useApplyFramework.ts`（add-blog-tiptap-editor 落地）
- 现有 `<Drawer>`：`src/components/shell/Drawer.tsx`（add-app-shell 落地）
- 现有 `useUIStore`：`src/stores/uiStore.ts`（drawer 状态字段）
- 现有 `useFrameworkStore`：`src/stores/frameworksStore.ts`（仅用于 Dexie 框架 CRUD；本 change 不依赖）
- 现有 `Framework` 模型：`src/types/domain.ts`
- BlogEdit 集成点：`src/pages/blogs/BlogEdit.tsx`（add-blog-tiptap-editor 落地）
- PlanDetail 侧 `FrameworkDrawer`：`src/features/framework/components/FrameworkDrawer.tsx`（计划侧；v1.0 与本 change 共存）
