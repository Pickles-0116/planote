# settings-and-shell 规范（增量 / Delta Spec）

> **Capability**：`settings-and-shell`
> **Change**：`add-settings-and-shell`
> **类型**：ADDED Requirements（全新能力）
> **来源**：PRD v1.0 §6.1「设置中心」+ §7「数据所有权」+ `useUIStore.theme` 已落地字段（v1.0 升级语义）+ `add-data-layer-dexie`（6 张 Dexie 表）+ `add-app-shell`（已落地 AppLayout / Sidebar / Header）+ `add-zustand-stores`（持久化基础设施）

本 capability 定义 Planote v1.0 收官的设置中心 + dark mode 全站适配 + 数据主控权（导入/导出/清除）的完整交互契约。
...
- `useUIStore.theme`：`src/stores/uiStore.ts`（v1.0 占位字段，语义升级）
- `Dexie db`：`src/db/schema.ts`（6 张表）
- `AppLayout` / `Sidebar` / `Header`：`src/components/layout/`
- Tailwind dark mode：`tailwind.config.ts`（需新增 `darkMode: 'class'`）
- 主题切换 hook：`src/features/settings/hooks/useTheme.ts`（本 change 新建）
- 数据 IO hook：`src/features/settings/hooks/useDataIO.ts`（本 change 新建）
- 持久化迁移：`src/stores/uiStore.ts` persist version 1 → 2
- 跨 store 同步：v1.0 简版——切换主题不影响 liveQuery 数据
- 设计参考：`add-app-shell`（统一外壳原语）+ `add-kanban-board`（dark 适配的复杂页面参考）
