# blog-list-and-detail 规范（增量 / Delta Spec）

> **Capability**：`blog-list-and-detail`
> **Change**：`add-blog-list-and-detail`
> **类型**：ADDED Requirements（全新能力）
> **来源**：PRD v1.0 §6（博客模块）+ `docs/ux-guidelines.md` §1 原则 4（密度自适配）+ `add-plan-list-view`（参考实现）+ `add-blog-tiptap-editor` + `add-blog-attachment`

本 capability 描述 Planote 博客列表页 `/blogs` 与详情页 `/blogs/:id` 的完整交互契约——卡片/列表双视图、搜索、框架/标签/状态筛选、排序、元数据展示、删除流程。
...
- `Blog` 模型：`src/types/domain.ts`（title / content / excerpt / tagIds / frameworkId / sourcePlanId / attachmentIds / status / createdAt / updatedAt）
- `BlogRepo`：`src/db/repos/BlogRepo.ts`（list / get / delete / search）
- `useBlog` / `useBlogs` hook：`src/stores/hooks/useBlog.ts` / `useBlogs.ts`（live query）
- `useBlogStore`：`src/stores/blogsStore.ts`（deleteBlog / updateBlog）
- `useFrameworks`：`src/stores/hooks/useFrameworks.ts`（4 个内置 framework 名称）
- `useTags`：`src/stores/hooks/useTags.ts`（标签 ID → 名称映射）
- `useUIStore`：`src/stores/uiStore.ts`（blogListView / blogListSort / blogListStatusFilter）
- `<RichEditor readOnly>`：`src/features/blog/components/RichEditor.tsx`（Round 8）
- `<AttachmentList>` + `<ImageLightbox>`：`src/features/blog/components/AttachmentList.tsx` + `src/shared/components/ImageLightbox.tsx`（Round 10）
- `<EmptyState>`：`src/components/shell/EmptyState.tsx`
- `countText`：`src/features/blog/utils/countText.ts`
- `extractPlainText`：`src/features/blog/utils/extractPlainText.ts`
- `formatChineseDate`：`src/lib/utils.ts`
- sort-engine 模式参考：`add-smart-sort`（Round 7）
- 视觉规范参考：`add-plan-list-view`（PlanCard / PlanListFilters / PlanListToolbar）
